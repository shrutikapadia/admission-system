...
from operator import itemgetter
 ...
 # ques -2 Give rank to the students so that admission process will be performed in proper sequence. Use any mechanism to give rank to the students.  #[Marks: 05]
 
def rank_students():
    student_results = []          # creating list for result of student.
    with open('student_details.csv', 'rt')as sd:
        data = csv.reader(sd)     # Fetching data from the csv file 
        for row in data:
            student_results += [row]  # adding row for percentage of student from marks of 5 subjects    
    ...
    student_results = sorted(student_results, key=itemgetter(6), reverse=True)  #sorting in reverse order so that one getting max marks is on top
...
#The above  ode gave rank to student. End of question 2.
...
# ques-4 generate day wise college status csv file and ques-5 if no seat is alloted it recheck next day and allot seat to student 
